...
class AAindexModelNetwork(nn.Module):
    def __init__(self, hyperparams):
        super(AAindexModelNetwork, self).__init__()

        self.hp = hyperparams

        self.conv1_feature = nn.Conv1d(
            in_channels=len(self.hp['aaindex_ids']),
            out_channels=self.hp['conv1_feature_out_channels'],
            kernel_size=self.hp['conv1_feature_kernel_size'],
            padding='same'
        )
        ...
        self.lstm = nn.LSTM(
            input_size=self.hp['conv1_feature_out_channels'] + len(KINGDOMS),
    ...
    def forward(self, x: torch.Tensor, kingdom) -> torch.Tensor:
        ...
        x_feature = self.conv1_feature(x_feature)
        x_feature = x_feature.transpose(2, 1)
        x_feature = F.relu(x_feature)

        kingdom = kingdom.expand(shape[0], shape[1], len(KINGDOMS))

        x = torch.cat((x_feature, kingdom), 2)
        x = self.dropout1(x)
...
class AAindexModel(BaseModel):
    def __init__(self, background, learning_rate, device):
        ...
        self.network = AAindexModelNetwork(self.hyperparams)
        self.network.to(self.device)

        super(AAindexModel, self).__init__(
            background,
            learning_rate,
            device,
            loss_weight={
