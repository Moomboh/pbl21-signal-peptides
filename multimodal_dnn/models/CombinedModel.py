...
class CombinedModelNetwork(nn.Module):
    def __init__(self, hyperparams):
        super(CombinedModelNetwork, self).__init__()
        ...
        self.lstm = nn.LSTM(
            input_size=self.hp['conv1_blosum62_out_channels'] +
            self.hp['conv1_feature_out_channels'] + len(KINGDOMS),
    ...
    def forward(self, x: torch.Tensor, kingdom) -> torch.Tensor:
        ...
        x = torch.cat((x_blosum62, x_feature, kingdom), 2)
        x = self.dropout1(x)
...
class CombinedModel(BaseModel):
    def __init__(self, background, learning_rate, device):
        ...
        self.network = CombinedModelNetwork(self.hyperparams)
